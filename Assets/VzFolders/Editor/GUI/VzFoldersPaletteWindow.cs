...
using static VzFolders.VzFoldersPalette;
...
namespace VzFolders
{
    public class VzFoldersPaletteWindow : EditorWindow
    {
            ...
            void colors()
            {
                ...
                void color(int i)
                {
                    ...
                    void colorOutline()
                    {
                        ...
                        var outlineColor = i <= VzFoldersPalette.greyColorsCount ? Greyscale(.0f, .4f) : Greyscale(.15f, .2f);
                    ...
                    void color()
                    {
                        ...
                        if (i <= VzFoldersPalette.greyColorsCount)
                        {
                        ...
                        var colorRaw = palette ? palette.colors[i - 1] : VzFoldersPalette.GetDefaultColor(i - 1);
        ...
        {
            if (VzFoldersData.storeDataInMetaFiles) return; // empties removed from meta files in SaveData()
        ...
        {
            if (!VzFoldersData.storeDataInMetaFiles)
                if (data)
                    data.RecordUndo();
        ...
        void MarkDatasDirty()
        {
            ...
            if (VzFoldersData.storeDataInMetaFiles)
                VzFolders.folderDatasFromMetaFiles_byGuid.Clear();

        }
        void SaveData()
        {
            if (!VzFoldersData.storeDataInMetaFiles) return;
            // if (!VzFoldersData.storeDataInMetaFiles) { data.Save(); return; }
        ...
        static void RepaintOnAlt() // Update 
        {
            if (curEvent.holdingAlt != wasHoldingAlt)
                if (EditorWindow.mouseOverWindow is VzFoldersPaletteWindow paletteWindow)
        ...
        public void Init(List<string> guids)
        {
            void createData()
            {
                if (VzFolders.data) return;

                VzFolders.data = ScriptableObject.CreateInstance<VzFoldersData>();

                AssetDatabase.CreateAsset(VzFolders.data, GetScriptPath("VzFolders").GetParentPath().CombinePath("VzFolders Data.asset"));

            }
            void createPalette()
            {
                if (VzFolders.palette) return;

                VzFolders.palette = ScriptableObject.CreateInstance<VzFoldersPalette>();

                AssetDatabase.CreateAsset(VzFolders.palette, GetScriptPath("VzFolders").GetParentPath().CombinePath("VzFolders Palette.asset"));
        ...
        static VzFoldersPalette palette => VzFolders.palette;
        static VzFoldersData data => VzFolders.data;
        ...
        {
            instance = ScriptableObject.CreateInstance<VzFoldersPaletteWindow>();
        ...
        public static VzFoldersPaletteWindow instance;
        ...
        public class FolderDataWrapper
        {
            public string iconNameOrGuid
            {
                get
                {
                    ...
                    else if (VzFoldersMenu.autoIconsEnabled && folderState.autoIconName != "")
                ...
                set
                {
                    if (VzFoldersMenu.autoIconsEnabled && folderState.autoIconName != "")
            ...
            {
                folderData = VzFolders.GetFolderData(guid, createDataIfDoesntExist: true);
                folderState = VzFolders.GetFolderState(guid);
            }
            ...
            // it's the only structural difference between VzFolders' PaletteWindow and vHierarchy's
